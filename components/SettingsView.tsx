    ...
    <div className={`flex flex-col h-full transition-colors duration-1000 ${settings.theme === Theme.CHRISTMAS ? 'bg-red-950' : 'bg-slate-950'} p-6 overflow-y-auto pb-48 relative`}>
      ...
      <div className="flex flex-col items-center mb-8 mt-6 z-10">
        <div className="relative">
          ...
          <Zap className={`w-12 h-12 relative z-10 ${settings.theme === Theme.CHRISTMAS ? 'text-white' : 'text-green-500'}`} />
        </div>
      ...
      <div className="space-y-8 max-w-md mx-auto w-full z-10">
        ...
        <div className="glass p-5 rounded-[32px] space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              ...
              { id: Theme.CHRISTMAS, icon: Snowflake, label: 'Kerst', color: 'bg-white', shadow: 'bg-slate-300' }
            ].map((t) => (
              <button key={t.id} onClick={() => updateSetting('theme', t.id)} className="btn-3d-wrap h-20">
                <div className={`btn-3d-top ${settings.theme === t.id ? `${t.color} text-black` : 'bg-slate-900/50 text-slate-500 opacity-60'}`}>
        ...
        <div className="glass p-5 rounded-[32px] space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <input type="number" value={settings.minStopSeconds} onChange={(e) => updateSetting('minStopSeconds', Math.max(1, Number(e.target.value)))} className="bg-black/40 border border-white/10 rounded-2xl p-4 text-white text-center font-bold" />
            <input type="number" value={settings.maxStopSeconds} onChange={(e) => updateSetting('maxStopSeconds', Math.max(settings.minStopSeconds, Number(e.target.value)))} className="bg-black/40 border border-white/10 rounded-2xl p-4 text-white text-center font-bold" />
          </div>
        </div>
      </div>

      <div className="mt-12 text-center pb-8 z-10">
        <p className="text-[10px] font-black uppercase tracking-[0.4em] text-white opacity-40 italic">© Corneel Konings</p>
      </div>

      <div className="fixed bottom-0 left-0 w-full p-8 bg-gradient-to-t from-slate-950/80 via-slate-950/50 flex flex-col items-center z-50">
        <button
          onClick={onStart}
          ...
          className="btn-3d-wrap h-20 max-w-md disabled:opacity-30"
        >
          <div className="btn-3d-top bg-white text-black font-game text-2xl uppercase italic tracking-tighter">START BEATSTOP</div>
